...
function saveGamePoints(npc, win) {
    if (npc > 0) {
        var data_result = {
            npc_kills: npc,
            win_game: win,
        ...
        if (userIsAuthenticated === true) {
            $.ajax({
                url: hostPort + "bomberman_save_points/",
                type: "POST",
                data: {
...
(function() {
  ...
  function step() {
    ...
    if(gameMode != Mode.MULTI_VERSUS && enemies.length == 0) {
      ...
      win += 1
      console.log('Победил bomberman', win, 'Убил врагов:', npc)
      saveGamePoints(npc, win);
      npc = 0
